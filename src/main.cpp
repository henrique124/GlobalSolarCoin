...
const string strMessageMagic = "GlobalSolarCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      {
        int64_t nSubsidy = 425000000 * COIN;
...
{  
    ...
    if(pindexBest->nHeight > 1576800)
    {
        nSubsidy = 95.12937595 * COIN; 
		return nSubsidy + nFees; 
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GlobalSolarCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
